...
/**
 * \fn isBinaryPGM(const char *filename)
 * \brief Returns true if the file is a binary PGM file, i.e. checks if P5 or P6 type image.
*/
NTTW_DLL_SYM int isBinaryPGM(const char *filename);
...
/**
 * \fn addImage(long *image1, long *image2, const int rows, const int cols)
 * \brief Adds image1 and image2 together and returns pointer to result. EXPERIMENTAL
*/
NTTW_DLL_SYM long* addImage(long *image1, long *image2, const int rows, const int cols);

/**
 * \fn flipImage(nttw_integer *image, const int rows, const int cols)
 * \brief Rotates the image 180 degrees and returns pointer to result. EXPERIMENTAL
*/
NTTW_DLL_SYM nttw_integer* flipImage(nttw_integer *image, const int rows, const int cols);
/**
 * \fn flipSignedImage(long *image, const int rows, const int cols)
 * \brief Rotates the signed image 180 degrees and returns pointer to result. EXPERIMENTAL
*/
NTTW_DLL_SYM long* flipSignedImage(long *image, const int rows, const int cols);
/**
 * \fn rotateImage90Degs(nttw_integer *image, const int rows, const int cols)
 * \brief Rotates the image 90 degrees and returns pointer to result. EXPERIMENTAL
*/
NTTW_DLL_SYM nttw_integer* rotateImage90Degs(nttw_integer *image, const int rows, const int cols);
/**
 * \fn rotateSignedImage90Degs(long *image, const int rows, const int cols)
 * \brief Rotates the signed image 90 degrees and returns pointer to result. EXPERIMENTAL
*/
NTTW_DLL_SYM long* rotateSignedImage90Degs(long *image, const int rows, const int cols);
//@}
