 ...
 * NTTW FNTT 2D Program
 * \file ntt.c
 * \brief FNTT 2D Program for the NTTW C Library.
 *
...
int main(int argc, char *argv[])
{
    ...
    int rows, cols, binaryInFile = FALSE, binaryFile = FALSE, dyadicTransform = TRUE;
    ...
    if(argc < 3)
    {
        printf(">| Usage: %s <filename> <output> [binary output?]\n",argv[0]);
        printf(">| filename is loaded and transformed to produce output.\n");
        printf(">| Files should be PGM formats.\n");
        printf(">| [binary output?] is a Boolean, Default: 0 - ASCII.\n");
        return EXIT_FAILURE;
    }
    if(argc > 3)
        binaryFile = atol(argv[3]);

    //--------------------------------------------
    ///Load Image
    //Check if binary
    binaryInFile = isBinaryPGM(argv[1]);
    //load
    if(!readPGM(&image,&rows,&cols,argv[1],binaryInFile))
    {
