/**
* Dyadic Inverse Number RT
* Computes the Dyadic NRT of an image provided
...
int main(int argc, char *argv[])
{
    ...
    int N, rows, cols, binaryFile = FALSE, normTransform = TRUE, dyadicTransform = TRUE;
    unsigned long long duration = 0;

    printf(">| Dyadic Inverse Number RT Program.\n");
    ...
    if(dyadicTransform)
        Isum = inrt_dyadic(image,result,N,normTransform);
    else
        Isum = inrt(image,result,N,normTransform);
